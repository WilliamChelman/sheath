...
@Component({
  ...
  imports: [NgIcon],
  viewProviders: [provideIcons({ phosphorArrowLeft, phosphorArrowRight, phosphorX })],
  template: `
    <div
    ...
    >
      ...
      <button
        ...
        (click)="closeTour.emit()"
        [attr.aria-label]="t('close')"
      >
        <ng-icon name="phosphorX" class="text-lg" />
      </button>
        ...
        <div class="flex gap-2">
          @if (canGoPrevious()) {
            <button class="btn btn-sm btn-outline" (click)="previous.emit()">
              <ng-icon name="phosphorArrowLeft" class="text-sm" />
              {{ t('previous') }}
            </button>
          }
          ...
          @if (canGoNext()) {
            <button class="btn btn-primary btn-sm" (click)="next.emit()">
              {{ t('next') }}
              <ng-icon name="phosphorArrowRight" class="text-sm" />
            </button>
          } @else {
            <button class="btn btn-primary btn-sm" (click)="closeTour.emit()">
              {{ t('finish') }}
            </button>
          }
        </div>
